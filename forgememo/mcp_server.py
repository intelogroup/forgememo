...
MAX_PRINCIPLE_LEN = 2000
MAX_PROJECT_LEN = 100
MAX_SESSION_LEN = 200
MAX_TAGS_LEN = 500
MAX_K = 50
MAX_MEMORIES_PER_CALL = 100
_PROJECT_RE = re.compile(r"^[\w\-./@ ]+$")


def _validate_project(project: str | None) -> str | None:
    """Validate and sanitize project_tag. Returns None or clean string.
    ...
    if len(project) > MAX_PROJECT_LEN:
        raise ValueError(f"project too long ({len(project)} > {MAX_PROJECT_LEN})")
    if not _PROJECT_RE.match(project):
        raise ValueError(f"project contains invalid characters: {project!r}")
    return project


def _err(code: str, message: str) -> str:
    """Return a structured JSON error string for agent consumption."""
